...
const Card = forwardRef<any, CardProps>((props, ref) => {
  ...
  const renderHeader = () => {
    ...
    return React.createElement(
      View,
      { style: headerStyles, className: "cross-card-header" },
      [
        typeof title === 'string' ?
          React.createElement(Text, { key: 'title', style: { fontWeight: 'bold', fontSize: 16 } }, title) :
          title,
        extra && React.createElement(View, { key: 'extra', className: "cross-card-extra" }, extra)
      ].filter(Boolean)
    );
  };

  // 渲染内容
  const renderBody = () => React.createElement(
    View,
    { style: bodyStyles, className: "cross-card-body" },
    children
  );
  ...
  const renderFooter = () => {
    ...
    return React.createElement(
      View,
      { style: footerStyles, className: "cross-card-footer" },
      footer
    );
  };
